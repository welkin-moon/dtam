...
class BrowserRoomHost {
  ...
  async attachPlayer(authoritySocket, params) {
    ...
    authoritySocket.send(JSON.stringify({
      ...
      self: { id: player.id, token: player.token, name: player.name }, features: { bushVision: true, mapManifest: '/maps/east-beach-v1.json', musicSync: true, roomIdentityV2: true, p2pFallbackV2: true }, hostId: room.hostId,
