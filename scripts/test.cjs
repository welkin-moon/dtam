...
includes(src,"function drawFrontPixelAnimal(x,y,r,animal",'players must use front-facing pixel-art animals');
includes(src,"ANIMAL_PIXEL_SHEET.src='/assets/animals/front-pixel-v1.png'",'player renderer must load the real pixel sprite sheet');
includes(worker,'this.chatHistory=[]','browser authority must persist bounded room chat history');
includes(worker,"channel!=='ghost'||dead",'chat history must preserve ghost privacy');
includes(src,'function loadChatHistory(list)','client must restore chat history on reconnect');
includes(src,"messages.some(m=>m.id===id)",'client must deduplicate chat messages by stable id');
assert(fs.existsSync('assets/animals/front-pixel-v1.png'),'pixel sprite sheet asset must exist');
...
includes(worker,"anonymousVoting:typeof raw.anonymousVoting==='boolean'",'browser authority must persist anonymous voting as a real room rule');
includes(worker,"const{votes,...safe}=result",'browser authority must strip voter identities from anonymous results');
includes(server,'obj.remove("votes")','Rust authority must strip voter identities from anonymous results');
...
includes(worker,'emergencyCooldown:clampInt(raw.emergencyCooldown,0,60','browser authority must validate emergency meeting cooldown');
includes(worker,"if(now<Number(this.emergencyReadyAt||0))",'browser authority must reject emergency calls during cooldown');
includes(server,'emergency_ready_at: i64','Rust authority must persist emergency meeting readiness');
includes(server,'now < rt.room.emergency_ready_at','Rust authority must reject emergency calls during cooldown');
includes(src,'id="ruleEmergencyCooldown"','lobby must expose emergency meeting cooldown');
includes(src,"Date.now()>=Number(gameState.emergencyReadyAt||0)",'client HUD must respect authoritative emergency readiness');
includes(src,'function pulseEvent(kind,text,duration=720)','gameplay events must expose lightweight visual/audio feedback');
...
includes(src,"wasAlive&&!selfState.alive",'meeting ejections must surface the ghost transition to the player');
includes(src,"pulseEvent('success','追踪已锁定')",'successful tracker activation must give immediate local feedback');
includes(src,"pulseEvent('shield','保护已施加')",'guardian protection must give immediate local feedback');
...
includes(src,"restartVoteBtn.onclick",'gameplay UI must expose restart voting');
includes(src,'function castWallVisionRay(angle,maxClientDist)','client must raycast wall visibility');
includes(src,'function drawWallVisionMask(view)','map itself must be occluded behind walls');
includes(src,'const wallVisionCache={','wall visibility must cache expensive raycasts');
includes(src,'function wallVisionRayCount()','wall visibility must scale ray count on lower-end clients');
includes(src,"Math.hypot(dx,dy)>=.12",'wall visibility cache must refresh after meaningful movement');
includes(src,'drawWallVisionMask(view);drawLightsMask(view)','wall occlusion must compose with light sabotage vision');
includes(html,'id="restartVoteBtn"','more menu must contain restart voting control');
