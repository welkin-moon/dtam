...
const p2pGamePatch=fs.readFileSync('p2p-game-room-patch.js','utf8');
...
includes(p2pResilience,'function iceCandidateCount(desc)','P2P failures must distinguish an empty ICE candidate set');
includes(p2pResilience,'请切换到 Auto','direct-only mode must explain the privacy-policy escape hatch');
includes(p2pResilience,"diag({ remoteIceCandidates })",'remote ICE candidate count must be exposed to diagnostics');
includes(p2pResilience,"diag({ localIceCandidates })",'local ICE candidate count must be exposed to diagnostics');
...
includes(p2pGamePatch,'const VOICE_SYNC_RETRY_MS = [1000, 2000, 4000, 8000, 15000, 30000]','voice sync failures must use bounded exponential backoff');
includes(p2pGamePatch,'if (state.inFlight || state.timer) return','voice sync calls must coalesce while a request or retry timer is active');
includes(p2pGamePatch,'if (!r.ok) throw new Error(`voice sync ${r.status}`)','non-2xx voice sync responses must enter retry backoff');
includes(p2pGamePatch,'0.8 + Math.random() * 0.4','voice sync retries must include jitter');
excludes(p2pGamePatch,'clearTimeout(state.timer)','voice sync state changes must not collapse an active retry backoff');
